...
def convert_to_int(data):
    ...
    for item in data:
        ap = []
        for subitem in item:
            ap.append(int(subitem))
        ret.append(ap)
    return ret

def read_csv_to_np(path : str, delimiter : str = ',', quotechar : str = '"') -> list[list]:
    """Donner le chemin vers un fichier csv et retourner un array numpy"""
    if os.path.isfile(path):
        if os.path.splitext(path)[1].lower() == ".csv":
            return genfromtxt(path, delimiter=delimiter)
        else:
